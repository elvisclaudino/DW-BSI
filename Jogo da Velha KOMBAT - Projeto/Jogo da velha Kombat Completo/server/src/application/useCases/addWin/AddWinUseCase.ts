...
class AddWinUseCase {
    ...
    // Método async de execução do UseCase de add Wins do usuário
    ...
    async execute({ username }: IRequest): Promise<IResponse> {
        ...
        const userUpdated = await this.usersRepository.updateWins(
            username,
            user.wins + 1
        );
        ...
        return new Created({ data: userUpdated, message: "win added" });
    }
}

export { AddWinUseCase };
